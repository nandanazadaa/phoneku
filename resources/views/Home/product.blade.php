...
<pre>
    Colors data: {{ print_r($colors, true) }}
    Is colors an array? {{ var_export(is_array($colors), true) }}
    Is colors empty? {{ var_export(empty($colors), true) }}
</pre>
    <div class="mb-4 flex items-center gap-2">
        ...
        <div class="flex items-center gap-2" id="color-options">
            @php
                // Ensure $selectedColor is initialized, this line is already in your code.
                $selectedColor = old('color', $selectedColor ?? (!empty($colors) && isset($colors[0]) ? trim($colors[0]) : ''));
            @endphp
            @foreach($colors as $color)
                @php
                    $cleanColor = trim($color);
                    // Use a helper function or a method on the product to get the color name
                    // For now, let's assume you'll add a method getFriendlyColorNameAttribute() to your Product model
                    $colorName = $product->getFriendlyColorName($cleanColor);
                @endphp
                @if(is_string($cleanColor) && preg_match('/^#[0-9A-Fa-f]{6}$/', $cleanColor))
                    <label class="w-6 h-6 rounded-full border-2 flex items-center justify-center cursor-pointer relative group"
                           style="border-color: {{ $selectedColor == $cleanColor ? '#3b82f6' : '#d1d5db' }};"
                           title="{{ $colorName }}"> {{-- Add title for hover tooltip --}}
                        <input type="radio" name="color" value="{{ $cleanColor }}" class="sr-only color-radio" {{ $selectedColor == $cleanColor ? 'checked' : '' }}>
                        <span class="block w-4 h-4 rounded-full" style="background: {{ $cleanColor }};"></span>
                        @if($selectedColor == $cleanColor)
            ...
            <div class="flex items-center gap-8 mt-4 mb-2">
                <div>
                    ...
                    <div class="flex items-center border border-gray-300 rounded-lg bg-white shadow-sm mt-1">
                        <button type="button" class="px-3 py-2 text-gray-600 quantity-btn minus-btn rounded-l-lg hover:bg-gray-50 hover:text-gray-800 transition-all duration-200 border-r border-gray-200 flex items-center justify-center" id="qty-minus">
                        ...
                        <button type="button" class="px-3 py-2 text-gray-600 quantity-btn plus-btn rounded-r-lg hover:bg-gray-50 hover:text-gray-800 transition-all duration-200 border-l border-gray-200 flex items-center justify-center" id="qty-plus">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
