...
function updateSelectedColorsDisplay($listElement, colorsArray, inputId) {
    ...
    colorsArray.forEach(color => {
        ...
        const displayColor = trimmedColor + (isValid ? '' : ' (Format Invalid)');
        const swatchHtml = isValid ? `<span class="color-swatch" style="background-color: ${trimmedColor};"></span>` : '';
        const itemClass = isValid ? 'selected-color-item' : 'selected-color-item text-danger border-danger';
    ...
    console.log(`Updated display for ${inputId} with colors:`, colorsArray);
}
...
function updateHiddenColorInput($hiddenInput, colorsArray) {
    ...
    console.log(`Updated hidden input ${$hiddenInput.attr('id')} with value:`, $hiddenInput.val());
}
...
$('.btn-add-color').on('click', function() {
    ...
    // Tentukan elemen berdasarkan data-target
    if (targetId === 'create') {
    ...
    if (newColor.length === 0) {
        alert('Masukkan warna terlebih dahulu.');
        return;
    }
    ...
    // Reset input ke warna default setelah ditambahkan
    if (isHexColor(newColor)) {
        $tempInput.val('#FFFFFF');
        try {
            $tempInput.closest('.colorpicker-element').colorpicker('setValue', '#FFFFFF');
            $tempInput.closest('.colorpicker-element').find('.input-group-text i').css('background-color', '#FFFFFF');
        } catch (e) {
            console.error('Error resetting colorpicker value:', e);
        }
    }
...
function handleTabContentInitialized(tabId) {
    ...
    if (tabId === 'create') {
        ...
        if (!$colorPickerElement.data('colorpicker')) {
...
$('form[id^="create-product-form"], form[id^="edit-product-form"]').on('submit', function(e) {
    const formId = $(this).attr('id');
    let $hiddenInput;

    if (formId === 'create-product-form') {
        $hiddenInput = $('#product_colors_input_create');
        updateHiddenColorInput($hiddenInput, selectedColorsCreate);
    } else if (formId === 'edit-product-form') {
        $hiddenInput = $('#product_colors_input_edit');
        updateHiddenColorInput($hiddenInput, selectedColorsEdit);
    }

    console.log('Form submitted with colors:', $hiddenInput.val());
});
